...
type MidiEvent = {
  deltaTicks: number;
  type: "noteOn" | "noteOff" | "tempo" | "other";
  noteNumber?: number;
  velocity?: number;
  tempoMicrosecondsPerQuarter?: number;
  // usado só pra manter ordem
};
 ...
 */
function readVLQ(data: Uint8Array, offset: number): { value: number; nextOffset: number } {
  let value = 0;
  let i = offset;
  for (;;) {
    if (i >= data.length) throw new Error("MIDI VLQ fora do buffer.");
    const b = data[i] & 0xff;
...
function parseMidiHeader(data: Uint8Array): { formatType: number; tracksCount: number; ticksPerQuarter: number } {
  // "MThd"
  if (data.length < 14) throw new Error("Arquivo MIDI muito curto.");
...
export function parseMidiToNoteEvents(midiBytes: Uint8Array): ParsedMidi {
  ...
  const activeNotes = new Map<string, { startTicks: number; midi: number }>();
  const channelPrograms = new Map<number, number>(); // channel -> program (1-indexed)
  let keySigSf   = 0;
  let timeSigNum = 4;
  let timeSigDen = 4;

  let firstTempoMicrosecondsPerQuarter: number | undefined;

  for (let trackIndex = 0; trackIndex < tracksCount; trackIndex++) {
    if (offset + 8 > data.length) throw new Error("Trilha MIDI fora do buffer.");

    const chunkId = String.fromCharCode(data[offset], data[offset + 1], data[offset + 2], data[offset + 3]);
    if (chunkId !== "MTrk") throw new Error(`Chunk MTrk não encontrado na trilha ${trackIndex}.`);
    ...
    while (offset < trackEnd) {
      ...
      if (eventByte === 0xff) {
        ...
        if (offset + metaLen > trackEnd) throw new Error("Meta event fora do buffer.");

        // tempo: 0x51, length 3
        if (metaType === 0x51 && metaLen === 3) {
          const mpq = (data[offset] & 0xff) << 16 | (data[offset + 1] & 0xff) << 8 | (data[offset + 2] & 0xff);
          if (firstTempoMicrosecondsPerQuarter === undefined) firstTempoMicrosecondsPerQuarter = mpq;
        }
      ...
      if (statusHigh === 0xc0) {
        const program = (data[offset] & 0xff) + 1; // 1-indexed
